...
function Header() {
  return <header className="site-header"><a className="brand" href="#/"><strong>MPM</strong><span>MODEL TO PRODUCT MANAGER</span></a><nav><a href="#/">模型洞察</a></nav></header>;
}
...
function Admin() {
  const [token, setToken] = useState(() => sessionStorage.getItem("mpm-admin-token") ?? "");
  const [runs, setRuns] = useState<RunSummary[]>([]);
  const [error, setError] = useState("");
  const [running, setRunning] = useState(false);
  const load = async () => {
    if (!token) return;
    try { setRuns(await listRuns(token)); setError(""); sessionStorage.setItem("mpm-admin-token", token); } catch (err) { setError((err as Error).message); }
  };
  useEffect(() => { void load(); }, []);
  const run = async () => {
...
function App() {
  ...
  return <><Header />{route.name === "home" ? <Home /> : route.name === "article" ? <Article slug={route.slug} /> : <Admin />}<footer>MPM · MODEL TO PRODUCT MANAGER · 官方事实与 AI 产品判断分层呈现</footer></>;
}
